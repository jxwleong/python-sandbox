...
def get_combination_in_list(*numberlist): 
    numberlist = append_self_if_single_list(*numberlist)
    combination_list = []
    for item in list(itertools.product(*numberlist)):
...
class Test_is_nestedlist_(unittest.TestCase):
    def test_is_nestedlist_given_not_nested_list_expect_false(self):
